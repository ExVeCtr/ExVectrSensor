...
/**
 * @brief A class implementing control for the MPU9250 9dof sensor. The MPU9250
 * has a Gyroscope, accelerometer and magnetometer.
 * @note Currently the magnetometer is not implemented.
 */
class MPU9250 : public VCTR::sensor::Accelerometer,
                public VCTR::sensor::Gyroscope,
                public VCTR::sensor::Magnetometer,
                public VCTR::sensor::Thermometer {
protected:
...
public:
   ...
   * @param ioBus Input/Output bus the sensor is connected to. Should be a
   * single connection not true bus.
  ...
  /**
   * @brief Implements reading IMU temperature values and publishing them to the
   * topic. These can be used to control the IMU temperature for better
   * accuracy.
   * @return true if successful, false otherwise.
   */
...
protected:
  int64_t realMeasurentTimestamp_ =
      0; // Last time the sensor was read. Set this to 0 after reading data.
         // Interrupts will update this.
  ...
  uint8_t srd_;
  bool akFailure_ = false;  // ################ Keep track of the AK8963 in case
                            // something fails.
  bool mpuFailure_ = false; // ################ Keep track of the MPU6500 in
...
public:
   ...
   * @param disableMag If true then mag will be disabled and not used.
   */
  MPU9250Driver(HAL::DigitalIO &ioBus, Core::Scheduler &scheduler,
                bool disableMag = false);

  /**
   * * @brief Attach this function to an interrupt event object for more
   ...
   * @note This is not the default setting and should be used with caution. Can
   * be too fast for some filters.
   ...
   * @note This is not the default setting and should be used with caution. Is
   * extremely fast for most filters.
