 ...
 *
 * Heavily modified for use with the ExVectrSensor library with some extra
 * features.
 *
 *
 */
...
BME280Driver::BME280Driver(HAL::DigitalIO &ioBus)
    : BME280Driver(ioBus, Core::getSystemScheduler()) {}

BME280Driver::BME280Driver(HAL::DigitalIO &ioBus, Core::Scheduler &scheduler)
    : Task_Periodic("BME280 Driver", 50 * Core::MILLISECONDS) {
  ioBus_ = &ioBus;
  scheduler.addTask(*this);
  // setPriority(1000);
}

void BME280Driver::taskInit() {
  if (ioBus_ == nullptr) {
...
//
//****************************************************************************//
bool BME280::initSensor(HAL::DigitalIO &ioBus) {

  if (ioBus.getInputType() != HAL::IO_TYPE_t::BUS_I2C &&
      ioBus.getInputType() != HAL::IO_TYPE_t::BUS_SPI) {
    VCTR::Core::printE("BME280 Connected to incorrect input type. Must be I2C "
                       "or SPI. Given type: %d.\n",
                       ioBus.getInputType());
    return false;
  }
  ...
  if (ioBus.getOutputType() != HAL::IO_TYPE_t::BUS_I2C &&
    ...
    VCTR::Core::printE("BME280 Connected to incorrect output type. Must be I2C "
                       "or SPI. Given type: %d.\n",
                       ioBus.getOutputType());
    return false;
  }
  ...
  if (chipID == 0x58) {
    ...
    VCTR::Core::printE("BME280 Driver: Chip ID: %d, is not correct! Could be a "
                       "connection issue.\n",
                       chipID);
...
bool BME280::readSensorData() {
  ...
  if (pressure != 0) {
    ...
    baroTopic_.publish(
        Core::Timestamped<DSP::ValueCov<float, 1>>(baroVal, time));
  }
  ...
  if (humidity != 0 && !noHumidity_) {
    ...
    hygroTopic_.publish(
        Core::Timestamped<DSP::ValueCov<float, 1>>(humVal, time));
  }
  ...
  if (temperature != 0) {
    ...
    thermTopic_.publish(
        Core::Timestamped<DSP::ValueCov<float, 1>>(tempVal, time));
  }

  return true;
}

bool BME280::readBaro() { return readSensorData(); }

bool BME280::readHygro() { return readSensorData(); }

bool BME280::readTherm() { return readSensorData(); }
...
float BME280::getTemperatureCorrection(void) { return settings.tempCorrection; }

void BME280::setHumidityOffset(float corr) { humidityOffset_ = corr; }

float BME280::getHumidityOffset(void) { return humidityOffset_; }

void BME280::setPressureOffset(float corr) { presOffset_ = corr; }

float BME280::getPressureOffset(void) { return presOffset_; }
