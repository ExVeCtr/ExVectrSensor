...
MPU9250Driver::MPU9250Driver(HAL::DigitalIO &ioBus, bool disableMag)
    : Task_Periodic("MPU9250 Driver", 1 * Core::MILLISECONDS) {
...
MPU9250Driver::MPU9250Driver(HAL::DigitalIO &ioBus, Core::Scheduler &scheduler,
                             bool disableMag)
    : Task_Periodic("MPU9250 Driver", 1 * Core::MILLISECONDS) {
  disableMag_ = disableMag;
  ioBus_ = &ioBus;
...
void MPU9250Driver::enable4kHzRate() {
  ConfigDlpf(DlpfBandwidth::DLPF_BANDWIDTH_250HZ_4kHz);
  setInterval(250 * Core::MICROSECONDS); // 4kHz rate
                                         // ConfigSrd(0);
}

void MPU9250Driver::enable32kHzRate() {
  ConfigDlpf(DlpfBandwidth::DLPF_BANDWIDTH_DISABLE_32kHz);
  setInterval(31.25f * Core::MICROSECONDS); // 32kHz rate
                                            // ConfigSrd(0);
}

void MPU9250Driver::interruptHandler() {

  realMeasurentTimestamp_ = VCTR::Core::NOW();
}

void MPU9250Driver::taskInit() {
  if (ioBus_ == nullptr) {
    Core::printE("MPU9250 taskInit(): ioBus is a nullptr. Give the "
                 "constructor the iobus connected with the sensor!\n");
    return;
  }
...
MPU9250::MPU9250(bool disableMag) { disableMag_ = disableMag; }

bool MPU9250::initSensor(HAL::DigitalIO &ioBus) {

  if (ioBus.getInputType() != HAL::IO_TYPE_t::BUS_I2C &&
      ioBus.getInputType() != HAL::IO_TYPE_t::BUS_SPI) {
    VCTR::Core::printE("MPU9250 Connected to incorrect input type. Must be "
                       "I2C or SPI. Given type: %d.\n",
                       ioBus.getInputType());
    return false;
  }

  if (ioBus.getOutputType() != HAL::IO_TYPE_t::BUS_I2C &&
      ioBus.getOutputType() != HAL::IO_TYPE_t::BUS_SPI) {
    VCTR::Core::printE("MPU9250 Connected to incorrect output type. Must be "
                       "I2C or SPI. Given type: %d.\n",
                       ioBus.getOutputType());
    return false;
  }
  ...
  VRBS_MSG("MPU9250 initSensor(): Sensor started! Default settings being set "
           "to 16G, 2000DPS, 184Hz DLPF and 1kHz sample Rate.\n");
...
bool MPU9250::readGyro() {
  ...
  if (!Read()) {
    VCTR::Core::printW(
        "MPU9250 readGyro(): Something went wrong reading sensor values!\n");
    return false;
  }
  ...
  gyroTopic_.publish(
      Core::Timestamped<DSP::ValueCov<float, 3>>(gyroVals, time));
...
bool MPU9250::readAccel() {
  ...
  if (!Read()) {
    VCTR::Core::printW(
        "MPU9250 readAccel(): Something went wrong reading sensor values!\n");
    return false;
  }
  ...
  accelTopic_.publish(
      Core::Timestamped<DSP::ValueCov<float, 3>>(accelVals, time));

  return true;
}

bool MPU9250::readMag() {
  if (!initialised_) {
  ...
  if (!Read()) {
    VCTR::Core::printW(
        "MPU9250 readMag(): Something went wrong reading sensor values!\n");
    return false;
  }
...
bool MPU9250::readTherm() {
  ...
  if (!Read()) {
    VCTR::Core::printW(
        "MPU9250 readTherm(): Something went wrong reading sensor values!\n");
    return false;
  }
  ...
  thermTopic_.publish(
      Core::Timestamped<DSP::ValueCov<float, 1>>(thermVals, time));

  return true;
}

bool MPU9250::readAll() {
  ...
  if (!Read()) {
    // VCTR::Core::printW("MPU9250 readAll(): Something went wrong reading
    // sensor values!\n");
    return false;
  }
...
/**
 * Below are implementations from original implementions but modified for
 * ExVectr.
 */
...
bool MPU9250::Begin(bool disableMag) {
  ...
  akFailure_ =
      disableMag;      // ####################### Reset in case it works again
  ...
  VCTR::Core::delay(VCTR::Core::MILLISECONDS *
  ...
  if (!akFailure_) {
    ...
    if (who_am_i != WHOAMI_AK8963_) {
      akFailure_ = true;
      VCTR::Core::printE(
          "MPU9250 mag WHOAMI check failed. WHOAMI: %d! Disabling mag!\n",
          who_am_i);
    }
  }
  /* Skip if mag failed */
  if (!akFailure_) {
    ...
    VCTR::Core::delay(VCTR::Core::MILLISECONDS *
                      100); // long wait between AK8963 mode changes
    ...
    mag_scale_[0] =
        ((static_cast<float>(asa_buff[0]) - 128.0f) / 256.0f + 1.0f) * 4912.0f /
        32760.0f;
    mag_scale_[1] =
        ((static_cast<float>(asa_buff[1]) - 128.0f) / 256.0f + 1.0f) * 4912.0f /
        32760.0f;
    mag_scale_[2] =
        ((static_cast<float>(asa_buff[2]) - 128.0f) / 256.0f + 1.0f) * 4912.0f /
        32760.0f;
    /* Set AK8963 to power down */
...
bool MPU9250::Read() {
  ...
  accel_mps2_[0] =
      static_cast<float>(accel_counts[1]) * accel_scale_ * 9.80665f;
  accel_mps2_[2] =
      static_cast<float>(accel_counts[2]) * accel_scale_ * -9.80665f;
  accel_mps2_[1] =
      static_cast<float>(accel_counts[0]) * accel_scale_ * 9.80665f;
  die_temperature_c_ =
      (static_cast<float>(temp_counts) - 21.0f) / temp_scale_ + 21.0f;
  gyro_radps_[1] = static_cast<float>(gyro_counts[0]) * gyro_scale_ *
                   3.14159265358979323846f / 180.0f;
  gyro_radps_[0] = static_cast<float>(gyro_counts[1]) * gyro_scale_ *
                   3.14159265358979323846f / 180.0f;
  gyro_radps_[2] = static_cast<float>(gyro_counts[2]) * gyro_scale_ * -1.0f *
                   3.14159265358979323846f / 180.0f;
